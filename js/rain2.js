...
var area = d3.svg.area()
    ...
    .y0(height*0.85-10)
    .y1(function(d) { return (height*0.85-10) - y(parseFloat(d[currState.toLowerCase()])) });
...
*/
    ...
    interface.append("line")
        ...
        .attr("y1",height*0.85-10)
        .attr("x2",timescale(ext[1]))
        .attr("y2",height*0.85-10)
        .style("stroke-width",2)
        .style("opacity",0.9)
        .style("stroke","#5095B2")
    ...
    for(var i = 0; i<5; i++) {
        interface.append("line")
            ...
            .attr("y1",height*0.85+i*20+10)
            .attr("x2",timescale(ext[1]))
            .attr("y2",height*0.85+i*20+10)
    ...
    interface.selectAll("disaster")
        ...
        .attr("y",function(e){
            if(e.EVENT_TYPE==="Hail") return height*0.85+5;
            else if(e.EVENT_TYPE==="Thunderstorm Wind") return height*0.85+25;
            else if(e.EVENT_TYPE==="Wildfire") return height*0.85+45;
            else if(e.EVENT_TYPE==="Drought") return height*0.85+65;
            else if(e.EVENT_TYPE==="Tornado") return height*0.85+85;
        })
    ...
    interface.append("line")
        .attr("x1",timescale(ext[0])-100)
        .attr("y1",height*0.9+60)
        .attr("x2",timescale(ext[1]))
        .attr("y2",height*0.9+60)
        .style("stroke","#333")
    ...
    var ticks = interface.selectAll(".timelegend")
        ...
        .attr("y2",15)
        .attr("transform","translate(0,"+(height*0.9+60)+")")
        .style("stroke","black")
    ...
    barlabels.enter().append("text")
        ...
        .attr("y",-6)
        .attr("text-anchor","middle")
        .attr("transform", function(d, i) { return "translate("+( width*0.8 + i * barWidth) + ","+(height*0.2)+")"; })
   ...
   cursorg.append("line")
    ...
    cursorg
        .attr("transform","translate("+(timescale(ext[0]))+","+(height*0.9)+")")
        ...
        .on("mousemove", function(){
            ...
            if(pressed) {
               ...
               cursorg
                    .attr("transform", "translate(" + (xdrag) + "," + (height * 0.9) + ")")
            }
        })
        .on("mouseup", function(){
    ...
    plotRain = function() {
        ...
        cursorg
            .attr("transform","translate("+(timescale(currDate))+","+(height*0.9)+")");
      ...
      /*  if(playing) {
            ...
            if (a !== "Month" && a !== "Day" && a !== "Year" && currRain[a]>0) {
                ...
                rain.append("circle")
                    ...
                    .attr("r", function (s) {
                        ...
                        return Math.sqrt((currRain[a] * 3000)/(2*Math.PI));
                        else return Math.sqrt((currRain[a] * 1000)/(2*Math.PI));
                    })
        ...
        interface.append("text")
            ...
            .attr("transform",  "translate("+( 110) + ","+(height*0.75)+")")
            .attr("fill","#333")
            .attr("font-size",20)
